...
M_start = 1 - P_start;
P_end = 0.6;
sd_mut_start = 0.4;  % standard deviation of Gaussian for mutation
sd_mut_end = 0.15;
intervalScalar = 1.05;   % scale the parent interval to create more variation 
...
%               P sd_mut  sd_rec N   NP   NC   intervalScalar score rank crowding
population= [   0.5, 0.1, 0.01, 20,  0.5, 0.95;
                0.6, 0.2, 0.03, 28,  0.8, 1.3;
                0.6, 0.05, 0.02,12,  1,   0.65;
                0.3, 0.15, 0.12, 30,  0.8, 0.9 ];
...
%     pause(5);
    ...
    if fixedPFlag == 0
        sd_mut = sd_mut_start/(it)^0.7; %with low population sizes, we might miss some optimal direction and it will get stuck
        P = 1 - M_start/(it)^0.7;
    else
        P = P_end;
        sd_mut = sd_mut_end;
    ...
    offspring=geneticOperators(parents,NC,P,intervalScalar,sd_mut,0,V,M,f,lb,ub);
...
%         %sense to plot when only one objective function
%         drawnow;
%         pause(5);
%     end
    
    [stopFlag, crowdingDistanceFlag] = stopCriterion(it, population(:,V+M+1),population, V+M+2, M);%oldPopulationObjectives, population(:,V+1:V+M), notChangingLimit, V, M);
    it=it+1;
